...
class AddOrEditMediumGoalDialog extends StatefulWidget {
  ...
  const AddOrEditMediumGoalDialog({
    super.key,
  ...
  @override
  State<AddOrEditMediumGoalDialog> createState() =>
      _AddOrEditMediumGoalDialogState();
}
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          TextField(
            ...
            decoration: const InputDecoration(hintText: '目標のタイトル'),
            autofocus: true,
          ),
            ...
            children: [
              Expanded(
                child: Text(
                  _selectedDate == null
                      ? '期限を選択'
                      : '期限: ${DateFormat('yyyy/MM/dd').format(_selectedDate!)}',
                ),
              ),
              IconButton(
      ...
      actions: <Widget>[
        ...
        TextButton(
          child: Text(widget.isEditMode ? '保存' : '追加'),
          onPressed: () {
            if (_titleController.text.isNotEmpty) {
              widget.onSave(_titleController.text, _selectedDate);
